...
/*
static double recall_at_k_traditional_nq(
        const faiss::idx_t* I, int I_k,
        int k,
        const faiss::idx_t* gt, int gt_stride,
        int nq) {
    // recall@k = |{returned top-k} ∩ {gt top-k}| / k averaged over queries
    double sum = 0.0;
    for (int qi = 0; qi < nq; qi++) {
        const faiss::idx_t* Ii = I + (size_t)qi * (size_t)I_k;
        const faiss::idx_t* gti = gt + (size_t)qi * (size_t)gt_stride;
        int hit = 0;
        for (int i = 0; i < k; i++) {
            faiss::idx_t id = Ii[i];
            for (int j = 0; j < k; j++) {
                if (id == gti[j]) { hit++; break; }
            }
        }
...
static faiss::idx_t* build_exact_gt(
    ...
    faiss::idx_t* I = new faiss::idx_t[nq * (size_t)k];
    float* D = new float[nq * (size_t)k];

    flat.search((faiss::idx_t)nq, xq, k, D, I);
...
int main(int argc, char** argv) {
    ...
    // Warmup
    {
        ...
        hnsw->search((faiss::idx_t)nq, xq, warm_k, Dw.data(), Iw.data());
    ...
    for (int ef : efs) {
        ...
        for (int k : k_list) {
            ...
            for (int rr = 0; rr < rounds; rr++) {
                double t1 = now_sec();
                hnsw->search((faiss::idx_t)nq, xq, kk, D.data(), I.data());
                double t2 = now_sec();
                double sec = t2 - t1;

                if (fids && (ids_cfg.round_all || rr == 0)) {
                    dump_ids_one_round_rowwise(fids, ef, kk, rr, I.data(), (int)nq);
                }
