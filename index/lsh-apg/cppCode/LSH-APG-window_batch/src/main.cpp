...
static std::vector<int> parse_k_list(const char* s) {
    std::vector<int> ks;
    if (!s) return ks;
    std::stringstream ss(s);
    std::string tok;
    while (std::getline(ss, tok, ',')) {
        if (!tok.empty()) ks.push_back(std::atoi(tok.c_str()));
    }
    if (ks.empty()) ks.push_back(std::atoi(s)); // 没逗号：当单个 k
    return ks;
}
...
int main(int argc, char const* argv[])
{
    ...
    std::string data_fold = "/hd1/workspace/LSH-APG/LSH-APG-main/dataset/", index_fold = "./indexes/";
	...
	Preprocess prep(data_fold + datasetName + ".data", data_fold + "ANN/" + datasetName + ".bench_graph", 3000u);
    ...
    for (auto& kk : ks) {
        k = (unsigned)kk;
        if (divG) divG->ef = k + 150;          // 你原来就这么设
        graphSearch(c, k, divG, prep, beta, datasetName, data_fold, 2, "divG", window_size);
        std::cout << std::endl;
	}
	...
	for (auto& ef : efs) {
		if (fsG) fsG->ef = ef;
		graphSearch(c, k, fsG, prep, beta, datasetName, data_fold, 0, "fastG", window_size);
	}
