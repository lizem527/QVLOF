...
public:
	...
	bool hasT = false;
	float beta = 0.1f;
    unsigned max_query_limit = 3000; // default keeps old behavior
    unsigned query_num = 0;         // actual query count split from *_new
...
public:
    ...
    Preprocess(const std::string& path, const std::string& ben_file_, unsigned max_query_limit_);
