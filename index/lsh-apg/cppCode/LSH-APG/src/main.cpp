...
int main(int argc, char const* argv[])
{
	...
	Preprocess prep(data_fold + datasetName + ".data", data_fold + "ANN/" + datasetName + ".bench_graph", 4885u);
	...
	for (auto& kk : ks) {
        ...
        graphSearch(c, k, divG, prep, beta, datasetName, data_fold, 2, "divG");
        std::cout << std::endl;
	}
	...
	for (auto& ef : efs) {
		if (fsG) fsG->ef = ef;
		graphSearch(c, k, fsG, prep, beta, datasetName, data_fold, 0, "fastG");
	}
