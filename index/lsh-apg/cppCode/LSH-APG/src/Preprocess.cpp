...
Preprocess::Preprocess(const std::string& path, const std::string& ben_file_, unsigned max_query_limit_)
{
    max_query_limit = max_query_limit_;
    lsh::timer timer;
    std::cout << "LOADING DATA..." << std::endl;
    timer.restart();
    load_data(path);
...
void Preprocess::load_data(const std::string& path)
{
	...
	//}
	//int MaxQueryNum = 200;
    unsigned mq = max_query_limit;
    if (mq == 0) mq = (unsigned)(data.N - 1); // 0 means "all but 1"
    int MaxQueryNum = min((int)mq, (int)data.N - 1);
    query_num = (unsigned)MaxQueryNum;
...
void Preprocess::ben_make()
{
    ...
    // after load_data(): data.N is base_N, query_num is query_N
    benchmark.N = (unsigned)query_num;
    benchmark.num = 100;
...
Preprocess::~Preprocess()
{
    ...
    clear_2d_array(data.query, data.N + query_num);
